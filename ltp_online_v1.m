function varargout = ltp_online_v1(varargin)
% LTP_ONLINE_V1 MATLAB code for ltp_online_v1.fig
%      LTP_ONLINE_V1, by itself, creates a new LTP_ONLINE_V1 or raises the existing
%      singleton*.
%
%      H = LTP_ONLINE_V1 returns the handle to a new LTP_ONLINE_V1 or the handle to
%      the existing singleton*.
%
%      LTP_ONLINE_V1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LTP_ONLINE_V1.M with the given input arguments.
%
%      LTP_ONLINE_V1('Property','Value',...) creates a new LTP_ONLINE_V1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ltp_online_v1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ltp_online_v1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ltp_online_v1

% Last Modified by GUIDE v2.5 15-May-2017 12:18:14

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @ltp_online_v1_OpeningFcn, ...
    'gui_OutputFcn',  @ltp_online_v1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before ltp_online_v1 is made visible.
function ltp_online_v1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ltp_online_v1 (see VARARGIN)

% Choose default command line output for ltp_online_v1
...
% UIWAIT makes ltp_online_v1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ltp_online_v1_OutputFcn(hObject, eventdata, handles)
...
function plot_motion_data(handles)
axes(handles.motion_plot);
if logical(get(handles.overlay,'Value'));
    if logical(get(handles.multi_color,'Value'))
        hold all;
    else
...
t = handles.data_motion_t +str2double(get(handles.time_offset,'String'));
y = handles.data_motion_var;

plot(t,y,'r')
xlimit = get(gca, 'xlim');
set(handles.ltp_slope_plot, 'xlim', xlimit);
hold on
...
handles.exp.slope = [];
handles.data_motion_var = [];
handles.data_motion_t = [];
...
while ~logical(get(handles.stop_watching,'Value'))
    ...
    bw = str2double(get(handles.motion_bw,'String'));
    kn = kn + 1;
    ...
    if mc >= round(bw/loop_period)
        mc = bw/loop_period;
        xy = [temp.data_xy{:}]';
        handles.data_xy = [xy(1:2:end) xy(2:2:end)];
        handles.data_t = ((double([temp.data_t{:}]')-handles.t0)*1e-6)/60; % uS to min
        handles.data_motion_var(end+1) = (handles.data_xy(end) - handles.data_xy(1))^2/handles.motion_bw;
        handles.data_motion_t(end + 1) = handles.data_t(end);
        plot_motion_data(handles)
...
function motion_bw_Callback(hObject, eventdata, handles)
% hObject    handle to motion_bw (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of motion_bw as text
%        str2double(get(hObject,'String')) returns contents of motion_bw as a double


% --- Executes during object creation, after setting all properties.
function motion_bw_CreateFcn(hObject, eventdata, handles)
% hObject    handle to motion_bw (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
